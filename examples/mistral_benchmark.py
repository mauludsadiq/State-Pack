#!/usr/bin/env python3
"""
State Pack benchmark on Mistral-7B-Instruct.
Measures KV cache reuse savings on a real production model.
Run: PYTHONPATH=. python3 examples/mistral_benchmark.py
"""
...
base_text = (
    "[INST] You are an autonomous legal research agent. "
    "Track instructions, prior tool results, citations, constraints, and open questions. "
    "Preserve full working memory and continue from prior state. [/INST]"
)
...
blob_path = list(__import__('pathlib').Path(STORE).glob(f"state_packet_{base_sha256}.pt"))
...
out_path = "demo/mistral_benchmark.json"
__import__('pathlib').Path("demo").mkdir(exist_ok=True)
__import__('pathlib').Path(out_path).write_text(json.dumps(result, indent=2))
